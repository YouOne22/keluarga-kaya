...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ setup_error?: string }> }) {
  ...
  const income=(monthTx||[]).filter((t:any)=>t.type==="income").reduce((s:number,t:any)=>s+Number(t.amount),0); const expense=(monthTx||[]).filter((t:any)=>t.type==="expense").reduce((s:number,t:any)=>s+Number(t.amount),0); const totalOpening=(accounts||[]).reduce((s:number,a:any)=>s+Number(a.opening_balance),0); const totalAllIncome=(allTx||[]).filter((t:any)=>t.type==="income").reduce((s:number,t:any)=>s+Number(t.amount),0); const totalAllExpense=(allTx||[]).filter((t:any)=>t.type==="expense").reduce((s:number,t:any)=>s+Number(t.amount),0); const balance=totalOpening+totalAllIncome-totalAllExpense;
